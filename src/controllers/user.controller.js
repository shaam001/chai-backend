...
const registerUser = asyncHandler(async (req, res) => {
    ...
    // const coverImageLocalPath = req.files?.coverImage[0]?.path;  // When we do not send a cover image, it throws an error. Therefore, it is better to use the method below.
...
const updateUserAvatar = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (!avatar.url) {
            throw new ApiError(400, "Error while uploading an avatar and getting the URL")
        }
        ...
        // Assignment: delete old avatar
        const avatarOldUrl = req.user?.avatar

        if (avatarOldUrl != "") {
            // it is better to store public_id in the database because the folder structure might change in the future, which could affect the URL
            const getPublicId = avatarOldUrl.split("/").pop().split(".")[0];
        ...
        if (avatar.url) {
            // If an error occurs in this block, the image will have already been uploaded, so we need to delete it from Cloudinary
            const getPublicIdOfNewImage = avatar.url.split("/").pop().split(".")[0];
            cloudinary.uploader.destroy(getPublicIdOfNewImage, { invalidate: true })
        }
...
const updateUserCoverImage = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (!coverImage.url) {
            throw new ApiError(400, "Error while uploading the cover image and getting the URL")
        }
        ...
        // Assignment: delete old cover image
        const coverImageOldUrl = req.user?.coverImage

        if (coverImageOldUrl != "") {
            // it is better to store public_id in the database because the folder structure might change in the future, which could affect the URL
            const getPublicId = coverImageOldUrl.split("/").pop().split(".")[0];
        ...
        if (coverImage.url) {
            // If an error occurs in this block, the image will have already been uploaded, so we need to delete it from Cloudinary.
            const getPublicIdOfNewImage = coverImage.url.split("/").pop().split(".")[0];
            cloudinary.uploader.destroy(getPublicIdOfNewImage, { invalidate: true })
        }
